...
const Sidebar: React.FC = () => {
    ...
    <div key={item.label} className="flex w-full gap-1 mb-1">
      <Link
        ...
        className={`flex-1 flex items-center justify-between h-8 px-2 rounded-lg transition-colors ${
          isActive 
            ? 'text-black bg-white' 
            : item.disabled 
              ? 'text-gray-400 opacity-50 hover:bg-gray-800' 
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className={`text-sm ${isActive ? 'text-black' : 'text-gray-300'}`}>
            {item.label}
          </span>
        </div>
        {item.badge && (
  ...
  const renderSection = (title: string, items: any[], sectionKey: keyof typeof expandedSections) => (
    <div>
      <button
        ...
        className="w-full px-2 flex items-center justify-between text-gray-300 rounded-lg hover:bg-gray-800 h-auto pt-2 mb-1"
      >
  ...
  return (
    <div className="sidebar py-2 w-60 h-full px-2 sidebar-black z-100">
      <div>
        <div className="transition-all duration-200">
      ...
      <button className="w-full flex items-center btn-primary-black text-medium h-10 mb-1 px-2 rounded-lg">
